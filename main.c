...
void display_temp(float temperature) {
  ...
  digit_1 = (uint8_t)(temperature / 10);
  // Prendo el digito 1
  ...
  digit_2= ((uint8_t)temperature)%10;
  // Prendo el digito 2
...
int main() {
  ...
  while (true) {
    ...
    float temperatura = 1 / (log(1 / ( VREF / (float)adc_reading - 1)) / BETA + 1.0 / 298.15)- 273.15; // temperatura en celsius
